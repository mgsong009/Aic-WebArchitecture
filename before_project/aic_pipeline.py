    ...
    def fit(self, texts):
        ...
        if self.prefer == "sbert":
            try:
                ...
                print(f"[Backend] ✓ SBERT 로드: {self.sbert_model_name} (device={self.model.device})")
                return
                
            except Exception as e:
                print(f"[Backend] ✗ SBERT 실패: {e}")
                print(f"[Backend] → TF-IDF로 폴백")
        ...
        try:
            ...
            print(f"[Backend] ✓ TF-IDF 초기화 (vocab={len(self.vectorizer.vocabulary_)})")
        except Exception as e:
