...
| 영역 | 요약 | 확인 | 후속 작업 |
| --- | --- | --- | --- |
| Frontend | 관리자 `AIC 분석 품질 모니터`를 latest 단일 analysis run 화면에서 benchmark 중심 화면으로 재구성했습니다. `adminAnalysisApi.js`와 Pinia store는 `POST /admin/benchmarks`, `GET /admin/benchmarks`, `GET /admin/benchmarks/{run_id}`, `GET /admin/benchmarks/compare` 계약을 사용하도록 바뀌었고, 화면은 최근 제출 50건 benchmark 실행, 실행 진행률, 완료 run 목록, baseline/optimized 선택, p50/p95/평균/실패율/fallback 비교, 단계별 runtime 합계, sample outlier를 표시합니다. 기존 단일 분석 재실행 UI와 latest run 중심 데이터 의존성은 제거했습니다. 완료된 TODO 항목도 제거했습니다. | `npm.cmd run build` 성공. Vite dev server에서 `/admin/analysis-quality` 접근 시 미인증 상태가 `/login`으로 리다이렉트되는 route guard 확인. 로컬 backend가 떠 있지 않아 실제 admin API 데이터 렌더링은 확인하지 못했습니다. | 실제 backend와 DB 테이블 반영 후 admin 계정으로 benchmark 실행/조회/비교 흐름을 한 번 end-to-end 확인해야 합니다. |
| Backend | baseline/optimized benchmark 비교 API를 추가했습니다. `GET /api/v1/admin/benchmarks/compare?baselineRunId=...&optimizedRunId=...`는 두 benchmark run의 dataset hash 일치 여부와 경고를 반환하고, `p50`/`p95`/평균 runtime, 실패율, fallback 비율, data health numeric 항목, 단계별 runtime 합계의 baseline/optimized/delta/percent change를 계산합니다. 공통 submission의 runtime 차이가 큰 sample outlier도 warmup 제외 기준으로 반환합니다. 완료된 TODO 항목도 제거했습니다. | `python -m compileall aic-backend\app` 성공. | 프론트 분석 품질 모니터 benchmark 중심 재구성 완료. |
| Backend | benchmark 진행률 및 결과 조회 API를 추가했습니다. `GET /api/v1/admin/benchmarks`는 최근 benchmark run 목록을 `status`, `processed`, `total`, 실패 수, warmup 제외 수, dataset hash, aggregate runtime/failure/fallback 지표와 함께 반환하고, `GET /api/v1/admin/benchmarks/{run_id}`는 단일 run의 dataset snapshot, pipeline/config/code version, 단계별 runtime 합계, data health 요약, item별 metric snapshot/runtime/status/error/embedding/pipeline steps를 반환합니다. 실행 중 run도 같은 계약으로 polling할 수 있습니다. 완료된 TODO 항목도 제거했습니다. | `python -m compileall aic-backend\app` 성공. | baseline/optimized benchmark 비교 API 완료. |
| Backend/Pipeline/DB | `POST /api/v1/admin/benchmarks` 실행 API를 추가했습니다. 요청 시 최근 제출 최대 50건을 `submitted_at`, `id` 내림차순으로 고정 snapshot/hash와 함께 `benchmark_runs`/`benchmark_run_items`에 저장하고, background task가 실제 pipeline을 순차 재실행해 운영 `metrics` 테이블은 갱신하지 않은 채 item별 metric snapshot, runtime, status/error, embedding backend, pipeline steps를 저장합니다. warmup 0-3건은 `is_warmup`으로 표시하고 aggregate 통계에서 제외하며, 실행 중 processed/failed 카운트와 완료 시 p50/p95/평균 runtime, 실패율, fallback 비율, 단계별 runtime 합계, data health 요약을 run에 저장합니다. 완료된 TODO 항목도 제거했습니다. | `python -m compileall aic-backend\app` 성공. | benchmark 진행률 및 결과 조회 API 완료. |
| Pipeline/Backend | pipeline wrapper에서 `Data Load`, `Preprocess`, `PI`, `Embedding`, `UI/OI`, `AIC fit`, `Validation` 단계별 실제 실행 시간을 측정해 `/analyze` 응답의 `pipeline_steps`로 반환하도록 했습니다. `compute_UI_OI` 내부의 embedding fit/transform 구간과 UI/OI 계산 구간을 분리 계측하고, backend 분석 저장 경로는 기존 15/55/25% 비율 추정 대신 pipeline 응답의 실제 단계 시간을 저장하며 `Save` 단계만 backend에서 측정해 추가합니다. 로컬 TF-IDF fallback 스모크에서 드러난 최신 scikit-learn `np.matrix` 호환성 문제도 centroid 반환값을 ndarray로 바꿔 보정했습니다. 완료된 TODO 항목도 제거했습니다. | `python -m compileall aic-backend\app aic-pipeline\app aic-pipeline\aic_pipeline.py` 성공. 로컬 `run_analysis` 스모크에서 TF-IDF fallback 응답의 `pipeline_steps` 반환 확인. | benchmark 실행 API에서 `pipeline_steps` 저장과 aggregate 단계별 runtime 합계 반영 완료. |
| Backend/DB | 최근 제출 50건 benchmark 결과 저장을 위한 `benchmark_runs`, `benchmark_run_items` SQLAlchemy 모델과 `init.sql` 테이블을 추가했습니다. run에는 label/status, dataset snapshot/hash, pipeline/config/code version, warmup 제외 수, aggregate runtime(`p50`, `p95`, 평균), 실패율, fallback 비율, 단계별 runtime 합계, data health 요약을 저장하고, item에는 submission 연결, warmup 여부, metric snapshot, runtime, status/error, embedding backend, pipeline steps를 저장하도록 분리했습니다. 완료된 TODO 항목도 제거했습니다. | `python -m compileall aic-backend\app` 성공. | 기존 Docker DB 볼륨에는 `benchmark_runs`, `benchmark_run_items` 테이블을 별도 ALTER/CREATE 또는 DB 재초기화로 반영해야 합니다. |
| Backend/DB | 관리자 `AIC 분석 품질 모니터`의 처리 수·유효 수·성공률 저장 방식을 고정값에서 분석 job/metric 기반 계산으로 보정했습니다. 실행 중 run은 `processed_rows=0`으로 시작하고, 완료/실패 시 job 상태와 이번 실행 이후 저장된 metric의 필수 점수 존재 여부를 확인해 `processed_rows`, `valid_rows`, `success_rate`, 샘플당 평균 런타임, readiness를 저장합니다. ORM과 `init.sql`의 `analysis_runs.processed_rows` 기본값도 0으로 맞췄습니다. | `python -m compileall aic-backend\app` 성공. | 실제 Docker DB가 기존 볼륨을 유지 중이면 `analysis_runs.processed_rows` 기본값 변경은 운영 DB에 별도 ALTER 적용이 필요합니다. |
